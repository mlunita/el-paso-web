          ...
          <Link 
            ...
            className={`group relative overflow-hidden w-full aspect-[2.5/1] rounded-2xl flex flex-col items-center justify-center text-lg font-bold transition-all duration-300 ${
              isActive 
                ...
                : "glass-card text-zinc-400 hover:text-white hover:border-violet-500/20"
            }`}
            style={{ animationDelay: `${(index + 1) * 100}ms` }}
          >
            {/* Active state: gradient background */}
            {isActive && (
              <div className="absolute inset-0 bg-gradient-to-br from-violet-600 via-violet-600 to-indigo-600 opacity-90" />
            )}
            
            {/* Active glow border effect */}
            {isActive && (
              <div className="absolute inset-0 rounded-2xl ring-1 ring-violet-400/50 animate-pulse-glow" />
            )}
            
            <div className="relative z-10 flex flex-col items-center gap-2">
              <Icon className={`w-8 h-8 transition-all duration-300 ${isActive ? "scale-110 text-white drop-shadow-lg" : "group-hover:scale-110 text-zinc-500 group-hover:text-violet-400"}`} />
            ...
            {!isActive && (
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-1/2 bg-violet-500/10 blur-2xl" />
              </div>
            )}
          </Link>
        );
