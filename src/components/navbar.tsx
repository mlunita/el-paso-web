    ...
    <nav className="sticky top-4 z-50 w-full glass-card-strong rounded-2xl shadow-lg shadow-black/30 mb-8 transition-all animate-border-glow">
      <div className="h-16 flex items-center justify-between px-4 sm:px-6 md:px-8 text-white">
        <Link href="/" className="flex items-center gap-3 font-black text-xl tracking-widest uppercase hover:opacity-80 transition-opacity group">
          ...
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-slate-200 hidden lg:inline-block text-base xl:text-xl">EL PASO, TEXAS: BORDER ROLEPLAY. Official</span>
        </Link>
        ...
        <div className="hidden md:flex gap-1 lg:gap-2 items-center">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative px-3 lg:px-4 py-2 text-sm font-semibold transition-colors duration-300 after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2 after:w-0 after:h-[2px] after:bg-[#a67c52] after:rounded-full hover:after:w-3/4 after:transition-all after:duration-300 ${pathname === link.href ? "text-white after:!w-3/4" : "text-zinc-400 hover:text-white"
                }`}
            >
              {link.label}
            </Link>
        ...
        <div className="md:hidden border-t border-white/10 px-4 pb-4 pt-2 flex flex-col gap-1 animate-fade-in-up">
          {navLinks.map((link) => (
            <Link
              ...
              className={`px-4 py-3 rounded-xl text-sm font-semibold transition-colors duration-200 ${pathname === link.href
