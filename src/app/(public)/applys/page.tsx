    ...
    <div className="flex flex-col gap-8 sm:gap-12 w-full">
      <div className="text-center md:text-left mb-2 animate-fade-in-up">
        ...
        <div className="flex items-center gap-3 mt-3 justify-center md:justify-start">
          ...
          <p className="text-zinc-500 text-base sm:text-lg">Meet the team or become part of it.</p>
        </div>
      </div>
          ...
          {staff.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">
              {staff.map((member, index) => (
                <div 
                  key={member.id} 
                  className="group glass-card p-4 rounded-2xl flex items-center gap-5 hover:border-[#a67c52]/25 transition-all duration-300 shadow-lg shadow-black/20 hover:-translate-y-0.5 animate-fade-in-up"
                  style={{ animationDelay: `${(index + 1) * 80}ms` }}
                >
                  <div className="relative">
                    <Avatar className="w-14 h-14 border-2 border-zinc-800 group-hover:border-[#a67c52]/50 transition-all duration-300 shadow-xl">
                      <AvatarImage src={member.image || ""} />
                      <AvatarFallback className="bg-gradient-to-br from-[#a67c52]/30 to-[#7ca982]/30 text-zinc-200 font-black">{member.name[0]}</AvatarFallback>
                    </Avatar>
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest text-[#a67c52]">{member.role}</span>
                  </div>
                </div>
              ))}
            </div>
