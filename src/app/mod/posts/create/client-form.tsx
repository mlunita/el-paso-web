  ...
  return (
    <form action={formAction} className="flex flex-col gap-6 max-w-2xl bg-zinc-950/50 p-6 rounded-xl border border-white/10">
      <div className="space-y-2">
        ...
        <Input id="title" name="title" required placeholder="Enter post title..." className="bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
      </div>

      <div className="space-y-2">
        <Label htmlFor="authorName" className="text-zinc-400 font-bold">Author Name</Label>
        <Input id="authorName" name="authorName" required placeholder="Enter author name..." className="bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
      </div>

      <div className="space-y-2">
        <Label htmlFor="coverImage" className="text-zinc-400 font-bold">Cover Image URL</Label>
        <Input id="coverImage" name="coverImage" value={coverImage} onChange={(e) => setCoverImage(e.target.value)} placeholder="https://..." className="bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
        {coverImage && (
      ...
      <div className="space-y-2">
        ...
        <Textarea id="content" name="content" required placeholder="Write your content here..." className="min-h-[200px] bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
      </div>

      <div className="flex items-center gap-3">
        <input type="checkbox" id="published" name="published" className="w-5 h-5 rounded border-white/10 bg-black/50 accent-[var(--ep-secondary)]" />
      ...
      <div className="pt-4 flex gap-4 border-t border-white/10">
        ...
        <Button type="submit" disabled={pending} className="flex-1 bg-[var(--ep-secondary)] hover:bg-[#6b9471] text-white font-bold">{pending ? "Saving..." : "Save Post"}</Button>
      </div>
    </form>
  );
}
