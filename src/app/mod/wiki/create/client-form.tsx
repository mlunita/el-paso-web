  ...
  return (
    <form action={formAction} className="flex flex-col gap-6 max-w-2xl bg-zinc-950/50 p-6 rounded-xl border border-white/10">
      ...
      <div className="space-y-2">
        ...
        <Input id="title" name="title" required placeholder="e.g. VIP Gamepass" className="bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
      </div>

      <div className="space-y-2">
        <Label htmlFor="section" className="text-zinc-400 font-bold">Section</Label>
        <select id="section" name="section" required className="w-full h-10 px-3 rounded-md bg-black/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-[var(--ep-secondary)]/50">
      ...
      <div className="space-y-2">
        ...
        <Textarea id="description" name="description" required placeholder="Describe this item..." className="min-h-[120px] bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
      </div>

      <div className="space-y-2">
        <Label htmlFor="image" className="text-zinc-400 font-bold">Image URL (Optional)</Label>
        <Input id="image" name="image" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} placeholder="https://..." className="bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
      ...
      <div className="space-y-2">
        ...
        <Input id="order" name="order" type="number" required defaultValue={0} className="bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50" />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === ",") { e.preventDefault(); addTag(); } }} placeholder="Type a tag..." className="bg-black/50 border-white/10 focus-visible:ring-[var(--ep-secondary)]/50 flex-1" />
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-2">
            {tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[var(--ep-secondary)]/20 text-[var(--ep-secondary)] text-xs font-semibold border border-[var(--ep-secondary)]/20">
                {tag}
      ...
      <div className="pt-4 flex gap-4 border-t border-white/10">
        ...
        <Button type="submit" disabled={pending} className="flex-1 bg-[var(--ep-secondary)] hover:bg-[#6b9471] text-white font-bold">{pending ? "Saving..." : "Save Item"}</Button>
      </div>
    </form>
  );
}
